...
// Conditional import: HTML vs stub implementation
import 'excel_exporter_web_impl_stub.dart'
    if (dart.library.html) 'excel_exporter_web_impl_html.dart' as web_impl;
...
Future<void> exportExcelWithInfo(
  ...
  await web_impl.downloadBytesForWeb(
    bytes,
    'measurement_data_with_info.xlsx',
    mimeType:
...
Future<void> exportExcelFromTemplate({
  ...
  final sanitized = _sanitizeExcelTemplateBytes(byteData.buffer.asUint8List());
  ...
  final fileBytes = excel.encode();
  if (fileBytes == null) {
    throw Exception('Failed to encode Excel from template');
  }
  final bytes =
      fileBytes is Uint8List ? fileBytes : Uint8List.fromList(fileBytes);
  await web_impl.downloadBytesForWeb(
    bytes,
    filename ?? 'template_output.xlsx',
    mimeType:
        'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet',
  );
}

// Some Excel templates contain styles that reference unsupported numFmtIds
// (e.g., 31) which the excel package asserts on. Replace such ids with a
// widely supported one (14) to avoid parse errors.
